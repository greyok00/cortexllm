...
Universal unified memory system for any MCP-compatible AI agent.
...
class CortexLLMMemory:
    """Unified memory system with hot/warm/cold tiers"""
    ...
    def set_hot(self, platform: str, messages: list):
        """Set hot memory for a platform"""
        hot_file = HOT_DIR / f"{platform}.json"
        hot_file.write_text(json.dumps(messages, indent=2))
    ...
    def get_warm(self) -> list:
        """Get warm (unified) memory"""
        warm_file = WARM_DIR / "unified.json"
        if not warm_file.exists():
            return []
        try:
            return json.loads(warm_file.read_text())
        except:
            return []
    ...
    def set_warm(self, messages: list):
        """Set warm memory"""
        warm_file = WARM_DIR / "unified.json"
        warm_file.write_text(json.dumps(messages, indent=2))
...
async def read_resource(uri: str) -> str:
    ...
    if uri == "cortexllm://memory/hot":
        ...
        for hot_file in HOT_DIR.glob("*.json"):
            try:
                all_hot[hot_file.stem] = json.loads(hot_file.read_text())
            except:
                pass
        return json.dumps(all_hot, indent=2)
    
    elif uri == "cortexllm://memory/warm":
        return json.dumps(memory.get_warm(), indent=2)
...
async def list_tools() -> list[Tool]:
    ...
    return [
        Tool(
            name="memory_read",
            description="Read from CortexLLM unified memory",
            inputSchema={
                "type": "object",
                "properties": {
                    "tier": {
        ...
        Tool(
            name="memory_write",
            description="Write to CortexLLM unified memory",
            inputSchema={
                "type": "object",
                "properties": {
                    "tier": {
