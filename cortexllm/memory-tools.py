...
WARM_MEMORY = CORTEXLLM_ROOT / "memory/warm/per_profile.json"
...
def load_state():
    ...
    if not STATE_FILE.exists():
        return {
            "last_save": {"openclaw": None, "opencode": None, "claude": None},
            "next_turn": "openclaw",  # Which platform saves to warm next
            "turn_order": ["openclaw", "opencode", "claude"],  # Round-robin order
...
def record_save(platform=None):
    ...
    # Switch turn to next platform in round-robin
    turn_order = state.get("turn_order", ["openclaw", "opencode", "claude"])
    try:
        current_idx = turn_order.index(plat)
        next_idx = (current_idx + 1) % len(turn_order)
        state["next_turn"] = turn_order[next_idx]
    except ValueError:
        # Platform not in turn order, default to first
        state["next_turn"] = turn_order[0]
...
def load_hot(platform=None):
    ...
    # Normalize keys: handle both uppercase Messages and lowercase messages
    if isinstance(data, dict):
        if "messages" in data and "Messages" not in data:
            data["Messages"] = data["messages"]
        if "platform" in data and "Platform" not in data:
            data["Platform"] = data["platform"]
        if "sessionid" in data and "SessionID" not in data:
...
def save_hot(session, platform=None):
    """Save hot memory atomically for specified platform - dict format"""
    ensure_dirs()
    hot_path = get_hot_path(platform)
    plat = platform or get_platform()

    # Dict format: {"platform": "...", "messages": [...]}
    messages = []
    for msg in session.get("Messages", []):
        entry = {
            ...
            "platform": msg.get("Platform", plat)
        }
...
def message_to_md(message):
    """Convert CortexLLM message to markdown string. Supports both old and new format."""
    # Check for new format (role/content/platform)
    if "role" in message and "content" in message:
        role = message.get("role", "")
        content = message.get("content", "")
        time_str = message.get("timestamp", "")[:19].replace("T", " ")
...
def get_recent(limit=20, platform=None):
    """Get recent messages from warm memory (per-profile) or fallback to hot memory"""
    # Try warm memory first (has all platforms merged)
    if WARM_MEMORY.exists():
        try:
            with open(WARM_MEMORY) as f:
                data = json.load(f)
...
def get_all_recent(limit=60):
    """Get recent messages from all platforms merged (openclaw, opencode, claude)"""
    all_messages = []

    for plat in ["openclaw", "opencode", "claude"]:
        try:
