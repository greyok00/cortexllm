...
class Brain:
    ...
    async def start(self) -> None:
        """Start the brain: begin draining the task queue."""
        if self.running:
            return
        self.running = True
        self._worker_task = asyncio.create_task(self._drain_queue())

    async def stop(self) -> None:
        """Stop the brain: cancel the queue drain and mark as stopped."""
        self.running = False
    ...
    # ------------------------------------------------------------------
    # Task queue drain (was missing - tasks were queued but never run)
    # ------------------------------------------------------------------

    async def _drain_queue(self) -> None:
        """Continuously pull tasks off the queue and execute them."""
        while self.running:
            try:
                item = await asyncio.wait_for(self._queue.get(), timeout=1.0)
                task: Task = item["task"]
                ctx: Dict = item.get("ctx", {})
                asyncio.create_task(self._execute(task, ctx))
    ...
    async def _execute(self, task: Task, ctx: Dict) -> None:
        ...
        try:
            ...
            for i, wid in enumerate(task.workers):
                result = await self._run_worker(wid, task)
    ...
    async def _run_worker(self, wid: str, task: Task) -> str:
        ...
        self.workers[wid].status = "busy"
        self.workers[wid].current_task = task.id

        try:
            timeout = self.cfg.get("brain", "task_timeout", default=300)
            result = await asyncio.wait_for(worker.run(task.input), timeout=timeout)
            self.workers[wid].tasks_completed += 1
            self.workers[wid].status = "idle"
            return result
        except asyncio.TimeoutError:
            self.workers[wid].tasks_failed += 1
            self.workers[wid].status = "error"
            raise
    ...
    def _task_to_dict(self, task: Task) -> Dict:
        return {
            ...
            "mode": task.mode.name.lower(),
            "workers": task.workers, "progress": task.progress,
            "result": task.result, "error": task.error,
            "created": task.created, "started": task.started, "done": task.done
        }
    ...
    def status(self) -> Dict:
        return {
            ...
            "workers": len([w for w in self.workers.values() if w.status == "busy"])
        }
