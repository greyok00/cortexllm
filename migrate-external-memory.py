...
def save_to_hot(messages):
    """Atomically save to hot memory. Cap at HOT_LIMIT."""
    ...
    session_data = {
        ...
        "Messages": [md_to_message(m["content"], source=m["source"]) for m in capped],
    ...
    print(f"\u2713 Written {len(capped)} messages to hot memory (cap {HOT_LIMIT}) \u2192 {hot_file.name}")
    return session_data


def save_to_warm(messages):
    """Atomically save to warm memory. Cap at WARM_LIMIT."""
    ...
    tmp_file.write_text(json.dumps([md_to_message(m["content"], source=m["source"]) for m in capped], indent=2))
